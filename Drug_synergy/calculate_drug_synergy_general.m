...
if      drug_synergy_opt.cell == 1
    load('dose-responses-icx-parental_general')
    
elseif  drug_synergy_opt.cell == 2
    load('dose-responses-icx-resistant_general')
end
sim_drug        = drug_response_icx_general.data;
% data structure
% d1: time
% d2: variable (pS6 and pS6K)
% d3: icx (n = 4)
% d4: icx (n = 4)
% d5: combo (n = 24)
% d6: readout (n = 2)
...
n3d     = length(markerproteins); % readout
n4d     = size(param_sets,2); % model
n5d     = length(my_variables); % (ps6 and ps6k)
...
R1_effect=reshape(single_R1,[n1d,n2d,n3d,n4d,n5d]);
R2_effect=reshape(single_R2,[n1d,n2d,n3d,n4d,n5d]);
R12_effect=reshape(comb_R12,[n1d,n2d,n3d,n4d,n5d]);
