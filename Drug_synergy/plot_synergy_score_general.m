...
% (4) {'phosphoRB' }
% (5) {'totalCd'   }
disp(strcat(' ==> ',my_variables{myvar_id},': selected for plot'));


for ii =1:size(CDI_score,3)
    
    dat_mat = [];
    dat_mat(:,:)    = real(log2(CDI_score(icx_idx,:,ii,:,myvar_id)));
    % data structure
    % d1: icx (4)
    % d2: combo (24)
    % d3: variables (2)
    ...
    num_samples_sorted = sum(~isnan(dat_mat(cdi_idx(ii,:),:)),2);
    ...
    title(strcat(my_variables{myvar_id},'-[',markerproteins{ii},']'))
    box off
    ...
    if DS_option.cell == 1
        ...
        writetable(tbl_cdi,fname,'WriteRowNames',true,'Sheet',strcat(my_variables{myvar_id},'-',markerproteins{ii}))
...
if DS_option.cell == 1
    ...
    % save the figure
    fname = strcat(fullfile(workdir,'Outcome'),'\drug-synergy-parental-CDI-',my_variables{myvar_id});
...
elseif  DS_option.cell == 2
    ...
    % save the figure
    fname = strcat(fullfile(workdir,'Outcome'),'\drug-synergy-resistant-CDI-',my_variables{myvar_id});
    saveas(fig_1,fname,'jpeg');
    saveas(fig_1,fname,'fig');
...
if DS_option.cell == 1
    % save the figure
    fname = strcat(fullfile(workdir,'Outcome'),'\drug-synergy-parental-drug_effect-',my_variables{myvar_id});
...
elseif DS_option.cell == 2
    % save the figure
    fname = strcat(fullfile(workdir,'Outcome'),'\drug-synergy-resistant-drug_effect-',my_variables{myvar_id});
    saveas(fig_2,fname,'jpeg');
    saveas(fig_2,fname,'fig');
end
