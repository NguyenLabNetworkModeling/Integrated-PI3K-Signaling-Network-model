...
for ii=1:length(My_RD_name)
    ...
    subplot(length(My_RD_name),1,ii),plot(log10(p21_range),nanmean((tmp_curve),2)','r')
    xlabel('p21(a.u.)'),ylabel(strcat(My_RD_name{ii},'(normal.)'))
    title(cell_label{CellType})
    axis tight
    pbaspect([4 3 1])
    ...
    file_name = strcat(workdir,'\Outcome\','p21KDnOE_',cell_label{CellType},'.xlsx');
...
fname_fig_2 = strcat(workdir,'\Outcome\',strcat('Signal Response to p21 expression_',cell_label{CellType},'.jpeg'));
